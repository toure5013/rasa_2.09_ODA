...
function send(message) {
	...
	$.ajax({
		url: 'http://172.31.240.6:5005/webhooks/rest/webhook',
		type: 'POST',
...
function setBotResponse(val) {
	setTimeout(function () {
		if (val.length < 1) {
			...
			msg = "Je suis d&eacute;sol&eacute; , je n'ai pas bien compris cela. Pourriez-vous reformuler?";
			//msg = "Je suis désolé , je n'ai pas bien compris cela. Pourriez-vous reformuler?";
			...
			for (i = 0; i < val.length; i++) {
				...
				if (val[i].hasOwnProperty("text")) {
					...
					if(val[i].text.indexOf("http") > -1 || val[i].text.indexOf("https") > -1){
					 var msg = val[i].text.split(" ");
					 var l = msg.length;
					 for ( var i = 0; i < l; i++ ) {
						if(msg[i].startsWith("http") || msg[i].startsWith("https")){
						msgTemp = "<a href='"+msg[i]+"' target='_blank'>"+msg[i]+"</a>";
						}else{
						msgTemp = msg[i];
						}
						messageFinal += msgTemp+" ";
					}

					}else {
						messageFinal = val[i].text;
					}

					var BotResponse = '<img class="botAvatar" src="./static/img/botAvatar.png"><p class="botMsg">' + messageFinal + '</p><div class="clearfix"></div>';
					$(BotResponse).appendTo('.chats').hide().fadeIn(1000);
				}
